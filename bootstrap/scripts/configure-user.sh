...
# Remove the retired fork build so the maintained Herdr installed by the
# dotfiles profile is the only binary on the host.
rm -f "$HOME/.local/bin/herdr"
rm -rf "$HOME/.local/share/herdr-custom"
